...
const SporeData = (()=>{
    ...
    SD.sporeArray = []
    SD.playerPointCount = Array(TheSettings.playerCount).fill(0)
    ...
    SD.calcDistance = function(x1,y1,x2,y2){
        ...
        var xDist = calcTorusDist(x1,x2,TheSettings.boardWidth)
        var yDist = calcTorusDist(y1,y2,TheSettings.boardHeight)
    ...
    SD.addNewSpore = function(sporeX,sporeY){
        ...
        try {
            SD.loopOverAllSpores(function(oldSpore) {
                if (SD.calcDistance(oldSpore.x,oldSpore.y,newSporeX,newSporeY) < TheSettings.innerRadius){
                    throw 'too close'
                }
            })

            SD.sporeArray.push(sporeConstructor(newSporeX,newSporeY,TheSettings.currentPlayer))
            ...
            SD.loopOverAllSpores(function(oldSpore){
                ...
                if (distanceToOtherSpore < TheSettings.outerRadius && distanceToOtherSpore !== 0){
                    SD.connectSpores(newCreatedSpore,oldSpore)
                }
            })
            TheSettings.nextPlayer()
    ...
    SD.checkIfColorsAreEqual = function(spore1,spore2){
        ...
        if(spore1.player === spore2.player){
            ...
            boardCtx.strokeStyle = TheSettings.neutralColor
        }
    }
...
//     }
    ...
    //     TheSettings.currentPlayer = SD.sporeArray[indexToDelete].player
    // }
    ...
    SD.checkWinner = ()=>{
        ...
        if (SD.calcScoreDiff() >= TheSettings.winningDiff){
            alert(`${TheSettings.playerNames[SD.calcLeadingPlayer()]} player wins!`)
        }
    }
