...
function createBoardShifter(keyOne,keyTwo,isXShifter){
    ...
    S.changeCoords = (coord,isReverse)=>{
        ...
        if(isXShifter){
            boardDimension = TheSettings.boardWidth
        }
        else{
            boardDimension = TheSettings.boardHeight
        }

        if (isReverse){
            return mod(coord - S.shift, boardDimension) + TheSettings.boardMargin
        }
        return mod( coord + S.shift , boardDimension) - TheSettings.boardMargin
    }
    ...
    S.shiftCoords = (sign, shiftLabel)=>{
        if(S.currentShiftLabel !== shiftLabel){
            ...
            S.shiftInterval = setInterval(() => {
                S.shift += TheSettings.shiftStep * sign
            },TheSettings.shiftInterval );
    ...
    S.clarShiftIntervalIfKey = (shiftLabel)=>{
        if (shiftLabel === S.currentShiftLabel){
            clearInterval(S.shiftInterval)
            S.currentShiftLabel = ''
            // console.log('clearTheShiftInterval')
        }
    }
